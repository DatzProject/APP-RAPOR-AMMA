...
const InputNilai = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    // Map setiap visibleHeader ke kategorinya
    const colCategories = visibleHeaders.map((header, idx) => {
    ...
    visibleHeaders.forEach((header, idx) => {
      ...
      const x = colPositions[idx + 1]; // +1 karena colPositions[0] = No
      ...
      >
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <input
            ...
            value={nilaiKKM}
            onChange={(e) => setNilaiKKM(e.target.value)}
            disabled={isSaving}
            style={{
              width: "100%",
              padding: "10px",
...
const InputTP = () => {
  ...
  const { tpData, refreshRekapData, updateLocalData, refreshMapelSheet } =
    useRekapData();

  // ✅ UBAH: Load data dengan row mapping
  useEffect(() => {
    console.log("InputTP mounted - forcing immediate refresh");
    setLoading(true);
    ...
    const refreshData = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch DataTP");
        }

        const jsonData = await response.json();

        // ✅ FILTER dan CREATE MAPPING
        if (jsonData.length > 0) {
          const headers = jsonData[0];
          const allRows = jsonData.slice(1);
          ...
          allRows.forEach((row: any, index: number) => {
            // Filter hanya row yang punya minimal 1 field terisi
            const hasData = Object.keys(headers).some((header) => {
          ...
          console.log(
            `📊 Filtered ${filteredData.length} rows with data from ${allRows.length} total rows`
          );
          console.log(`🗺️ Row mapping:`, mapping);
        } else {
          setData(jsonData);
        ...
        console.error("Error refreshing InputTP:", err);
        setError(err instanceof Error ? err.message : "Unknown error");
        setLoading(false);
      }
    };
  ...
  const filteredData = actualData.filter((row: any) => {
    ...
    return matchMapel && matchSemester;
  });
...
const DataEkstrakurikuler = () => {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      // Kembalikan tombol simpan SEKARANG
      setIsSaving(false);

      // Refresh data di background (tidak ditunggu)
      refreshRekapData(true);
    } catch (err) {
      console.error("=== ERROR DETAILS ===");
      console.error(err);
...
const DataSiswa = () => {
  ...
  const loadDataFromServer = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch DataSiswa");
      }

      const jsonData = await response.json();
      setData(jsonData);
      setLoading(false);
      console.log("✅ DataSiswa loaded from server");
    } catch (err) {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      // Reload dari server untuk memastikan data terbaru
      await loadDataFromServer();

      setIsSaving(false);

      // Background refresh untuk data lainnya
      setTimeout(() => {
        refreshRekapData(true);
      }, 2000);
    } catch (err) {
  ...
  const handleAddNew = async () => {
    ...
    try {
      ...
      // Optimistic update untuk mapel sheets
      const affectedSheets = addSiswaToMapelSheets(namaSiswa, kelas, nis, nisn);

      alert(
        `✅ Data siswa "${namaSiswa}" berhasil ditambahkan!\n\n` +
          `📝 Data ditambahkan ke ${affectedSheets} sheet mapel kelas ${kelas}.`
      );
  ...
  const handleDelete = async (rowIndex: number) => {
    ...
    try {
      ...
      if (hapusNilai) {
        const affectedSheets = removeSiswaFromMapelSheets(namaSiswa, kelas);
        alert(
          `✅ Data siswa "${namaSiswa}" berhasil dihapus!\n\n` +
            `🗑️ Nilai dikosongkan dari ${affectedSheets} sheet mapel kelas ${kelas}.`
        );
      } else {
        alert(
          `✅ Data siswa "${namaSiswa}" berhasil dihapus dari Data Siswa.\n\nNilai di Input Nilai tetap ada.`
        );
      }
    ...
    const load = async () => {
      ...
      try {
        ...
        const [kehadiranRaw, kokurikulerRaw, ekstraRaw] = await Promise.all([
          ...
          fetch(`${endpoint}?sheet=DataKokurikuler`).then((r) =>
            r.ok ? r.json() : null
          ),
          fetch(`${endpoint}?sheet=DataEkstrakurikuler`).then((r) =>
            r.ok ? r.json() : null
          ),
        ]);
...
const RekapNilai = () => {
  ...
  const downloadRaporPDF = async (rowData: any) => {
    ...
    try {
      ...
      const deskripsiPromises = semesterSheets.map(async (sheet) => {
        try {
          const response = await fetch(`${endpoint}?sheet=${sheet.sheetName}`);
          if (!response.ok)
      ...
      const [kokurikulerResult, ekstrakurikulerResult, kehadiranResult] =
        await Promise.all([
          fetch(`${endpoint}?sheet=DataKokurikuler`)
